(function(){
  ...
  */
  function calculator(operator){
    if(operation[operator] !== undefined){
      return function(x,y){
        if(!typeof Number(x) && !typeof Number(y)){
          return false;
        } else {
  ...
  */
  operationSignal = 'hey you!';
  var random = calculator(operationSignal);
  if(random){
    number1 = 10;
    number2 = 5;
    console.log(showOperationMessage(operationSignal,number1,number2), random(number1,number2));
  } else {
    console.log(showErrorMessage(operationSignal));
  }
